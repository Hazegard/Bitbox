...
if (file_exists("/var/www/html/bitbox/config.php"))
	include_once("/var/www/html/bitbox/config.php");
else if (file_exists("/var/www/html/bitbox/libs/config.php"))
	include_once "/var/www/html/bitbox/libs/config.php";
else
...
echo '<pre>'.print_r($dbh->query($sql),true).'</pre>';
	$num = $res->rowCount();
	$dbh = null;
